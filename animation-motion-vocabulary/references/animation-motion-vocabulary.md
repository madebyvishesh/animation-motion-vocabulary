...
Core vocabulary source: [Animation Vocabulary](https://animations.dev/vocabulary) by Emil Kowalski. This skill adapts it for AI agent use with additional matching and implementation guidance.

Use this reference to understand, suggest, and apply UI animation terms. If a user's prompt closely matches a term but does not name it, infer the term when obvious. If multiple terms could fit, ask a short confirmation question using the likely term names.

## Table of Contents

- [How to Use This Vocabulary](#how-to-use-this-vocabulary)
- [Near-Match Phrase Map](#near-match-phrase-map)
- [Entrances and Exits](#entrances-and-exits)
- [Sequencing and Timing](#sequencing-and-timing)
- [Movement and Transforms](#movement-and-transforms)
...
- [Looping and Ambient Motion](#looping-and-ambient-motion)
...
When the user's wording is vague, map their intent to one or two terms:

- "snappy", "responsive", "not sluggish" usually means ease-out or a well-damped spring.
- "appear", "come in", "show up" usually means fade in, slide in, scale in, pop in, reveal, or enter animation.
- "disappear", "leave", "remove smoothly" usually means fade out, slide out, scale out, reveal out, or exit animation.
- "one by one", "cascade", "wave of items" usually means stagger.
- "smooth between screens", "keep track of where it went" usually means continuity transition, shared element transition, or view transition.
- "comes out of the button", "opens from there" usually means origin-aware animation and transform origin.
- "appears while scrolling" usually means scroll reveal.
- "moves with scroll" usually means scroll-driven animation.
- "background moves slower" usually means parallax.
- "feels like iOS overscroll" usually means rubber-banding.
- "number rolls/counts" usually means number ticker with tabular numbers.
- "loading shine" usually means skeleton/shimmer.
- "text changes character by character" usually means text morph or typewriter depending on whether the old text transforms or new text appears sequentially.
- "stutters", "laggy", "not smooth" usually means jank, dropped frames, layout thrashing, or missing compositing.

Ask confirmation when the difference matters:

- Fade vs reveal: opacity change vs content being uncovered with a mask or clip-path.
- Slide in vs translate: entrance pattern vs the underlying transform used to move the element.
- Stagger vs orchestration: repeated delay between similar items vs deliberate timing across several different animations.
- Crossfade vs morph: same-location fade swap vs shape/content transforms.
- Shared element transition vs page transition: one element keeps identity vs the whole route animates.
- Scroll reveal vs scroll-driven animation: trigger on entry vs progress tied to scroll.
- Pulse vs idle animation: attention cue vs ambient life.
- Typewriter vs text morph: new text typed in vs existing characters animate into new characters.
